...
网页运行图（PC端/移动端）：
<p align="center">
  <img src="https://raw.githubusercontent.com/NewComer00/NJUST_HomeworkCollector/assets/web_page.png" alt="网页预览图" width=720>
</p>
...
```
注：  
1. 在执行start命令后，Nginx可能会提示一些文件或目录无法找到。执行```ps -a | grep nginx```来查询是否存在Nginx相关进程，若存在则Nginx已经启动。  
